...
void UART_init(){
   ...
   TIMSK0 |= (1<<OCF0A);
   //set compare value to 79 to achieve a 115200 baud rate (approximately 83.3µs)
   ...
   OCR0A = 79; // Seems to work well with 115200 baud rate (should be around 83.3µs, but the internal clock is a little iffy)
   //OCR0A = 160; // Seems to work well with 57600 baud rate (should be around  166.6µs, but the internal clock...)
   //enable interrupts
   sei();
}
